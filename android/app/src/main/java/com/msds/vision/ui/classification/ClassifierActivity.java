...
import com.msds.vision.ui.classification.env.BorderedText;
  ...
  private void recreateClassifier(Classifier.Model model, Classifier.Device device, int numThreads) {
    ...
    if (device == Classifier.Device.GPU
        && (model == Classifier.Model.QUANTIZED_MOBILENET || model == Classifier.Model.QUANTIZED_EFFICIENTNET ||
            model == Classifier.Model.QUANTIZED_DOGS_DENSENET || model == Classifier.Model.QUANTIZED_BIRDS_DENSENET)) {
      LOGGER.d("Not creating classifier: GPU doesn't support quantized models.");
      getActivity().runOnUiThread(
          () -> {
            Toast.makeText(getActivity(), R.string.tfe_ic_gpu_quant_error, Toast.LENGTH_LONG).show();
          });
      return;
    }
    try {
